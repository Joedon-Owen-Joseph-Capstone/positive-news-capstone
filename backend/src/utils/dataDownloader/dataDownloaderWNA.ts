...
function dataDownloader() : Promise<any> {
    ...
    async function downloadArticles() {
        try {
            ...
            // Search filters: SENTIMENT-0.69, LANGUAGE-english, PUBLISH AFTER-2023/01/01, NEWS SOURCES: Forbes & NBC & CNN & News Week & Google News & US News & CBS & AP News & NY Times & USA Today & PBS News & MSNBC & Washington Street Journal & Bloomberg & UN News & Buzz Feed & IGN & TODAY & WIRED & Reuters & Esports.net
            const {data} = await axios.get("https://api.worldnewsapi.com/search-news?number=150&min-sentiment=0.69&language=en&earliest-publish-date=2023-01-01&news-sources=https://www.forbes.com,https://www.cbsnews.com,https://www.vox.com,https://www.theguardian.com/us", {headers:{"x-api-key":process.env.NEWS_KEY}})
            ...
            for(let currentArticle of data.news) {
                ...
                if (currentArticle.author && currentArticle.image && currentArticle.source_country && currentArticle.id && currentArticle.url && currentArticle.title && currentArticle.text) {
